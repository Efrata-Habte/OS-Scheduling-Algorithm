...
int main() {
    ...
    /* Store number of requests for each process */
    int *request_counts = malloc(num * sizeof(int));
    ...
    Request *pending = malloc(total_requests * sizeof(Request));
    Request *valid = malloc(total_requests * sizeof(Request));
    Request *service_order = malloc(total_requests * sizeof(Request));
    int *waiting_time = malloc(total_requests * sizeof(int));
    ...
    for (int i = 0; i < total_requests; i++) {
        ...
        if (pending[i].sector < 0 ||
            pending[i].sector >= sector_count) {
    ...
    while (valid_count > 0) {
        ...
        Request *available =
            malloc(valid_count * sizeof(Request));

        int *available_indices =
            malloc(valid_count * sizeof(int));
        ...
        int actual_index =
            available_indices[local_index];

        service_order[serviced] = near;

        waiting_time[serviced] =
            current_time - near.arrival_time;

        int movement =
            abs(current_pos - near.sector);
    ...
    if (serviced > 0) {
        ...
        ave_waiting_time =
            (float)total_waiting_time / serviced;
    }

    if (current_time > 0) {
        throughput =
            (float)serviced / current_time;
    }
